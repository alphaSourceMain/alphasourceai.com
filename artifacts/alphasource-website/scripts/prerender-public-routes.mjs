...
const compiledContent = await build({
  entryPoints: [path.join(projectRoot, "src/lib/publicContent.ts")],
  absWorkingDir: projectRoot,
  alias: { "@": path.join(projectRoot, "src") },
  bundle: true, format: "esm", platform: "node", target: "node20", write: false,
});
const publicContent = await import(`data:text/javascript;base64,${Buffer.from(compiledContent.outputFiles[0].text).toString("base64")}`);
const LAST_UPDATED = publicContent.PUBLIC_CONTENT_LAST_UPDATED;
...
const faqItems = publicContent.publicFaqItems;
const supportQuestions = publicContent.publicSupportQuestions;
